...
import { getAllSets } from '../utils/workoutHelpers';
        ...
        case 'exercise_weight':
          if (achievement.requirement.exerciseId) {
            const exerciseSets = workouts
              .flatMap(w => getAllSets(w, { completedOnly: true }).filter(s => 
                s.exerciseId === achievement.requirement.exerciseId
              ));
            progress = exerciseSets.length > 0 ? Math.max(...exerciseSets.map(s => s.weight || 0)) : 0;
            isUnlocked = progress >= achievement.requirement.value;
          }
          break;
